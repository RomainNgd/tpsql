...
function getTypeNameToDelete($idType){
    $dbh = getConnexion();
    $req = 'SELECT CONCAT(idType, " : ",libelle) AS monType FROM types WHERE idType = :idType';
...
function deleteCours($idCours): bool
{
    $dbh = getConnexion();
...
function deleteType($idType): bool
{
    $dbh = getConnexion();
...
function updateCours($idCours,$libelle,$description, $idType, $nomImage): bool
{
    $dbh=getConnexion();
...
function addCours($libelle, $description, $idType, $image): bool
{
    $dbh=getConnexion();
